...
<tbody>
  @forelse ($full as $f) 
    @if(isset($prev_full) && $prev_full>$f->right_full)
      <?php $rank++; ?>
    @endif
    <tr class="rank_tr" style="background-color:rgb({{$f->r}},{{$f->b}},{{$f->g}})">
...
<tbody>
  @forelse ($part as $p) 
    @if(isset($prev_part) && $prev_part>$p->right_part)
      <?php $rank++; ?>
    @endif
<tr class="rank_tr" style="background-color:rgb({{$p->r}},{{$p->b}},{{$p->g}})">
      <td class="rank_td">{{ $rank }}</td>
...
<tbody>
  @forelse ($withnum as $w) 
    @if(isset($prev_withnum) && $prev_withnum>$w->right_withnum)
      <?php $rank++; ?>
    @endif
    <tr class="rank_tr" style="background-color:rgb({{$w->r}},{{$w->b}},{{$w->g}})">
      <td class="rank_td">{{ $rank }}</td>
